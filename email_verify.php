...
if(mysqli_num_rows($fire)>0)
{
  ...
  date_modify($rowsignuptime,"+10 minutes");
  $timeup = date_format($rowsignuptime,'d-m-Y h:i:s');
  $a = strtotime($timeup);
 // echo $a;
  //echo '<br>';
  $b = strtotime(date('d-m-Y h:i:s'));
//  echo $b;
  ...
  if($rowotp!==$otp)
  {
    ...
    if($b>=$a)
    {
      echo "<script>alert('your time up')</script>";
      header("Refresh:1; url= index.php");
    }
     else {
      $queryupdate = "UPDATE users set status = 'Active',otp='' where otp='$otp' and activation_code = '$activation_code'";
      $fireupdate = mysqli_query($con,$queryupdate);
      if($fireupdate) {
               echo "<script>alert('your Account Successfull Activated')</script>";
      header('location:add-appointment.php?code=' .$activation_code);
      }
      else{
                   ...
                   <form action="" method="POST">
                    ...
                    <div class="col-xs-6 col-sm-6 col-md-6">
                            <div class="form-group">
                              <label>OTP</label>
                              <input type="text" name="otp" id="otp" class="form-control input-md" required="" placeholder="Enter OTP">
                            </div>
                          </div>
                        </div>
